...
:::{.callout-tip title="interpretace"}
80 % hodnot je pod `r round(e35, 4)`.
:::
...
for (i in seq(from = 2150, to = 2250)) {
  set.seed(i)
  ests <- numeric(k)
  for (j in seq_len(k)) {
...
grid <- seq(from = 0, to = 1, length.out = 1e3)
...
rozdělení na data sedí hůře. Pořád ale výsledky dávají smysl.
:::
...
ggplot() +
  geom_histogram(aes(x = boys2sim)) +
  geom_vline(aes(xintercept = boys2))
```

:::{.callout-tip title="interpretace"}
Pozorovaný počet chlapců, co se narodili jako druzí, už popsané posteriorním rozdělením dobře není. To může znamenat, že první a druhý porod není nezávislý.
:::